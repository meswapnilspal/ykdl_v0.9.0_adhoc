...
DROP VIEW IF EXISTS shelfon_vw_simple_top10_multiBrand_add_option;

CREATE VIEW IF NOT EXISTS shelfon_vw_simple_top10_multiBrand_add_option
AS
	...
	LEFT JOIN (select * from shelfon_collected_option_data_processed where active_flag = 'Y') AS SCOD ON SCD.shelfon_data_seq = SCOD.shelfon_data_seq
