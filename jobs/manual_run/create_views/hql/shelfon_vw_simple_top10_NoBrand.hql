...
DROP VIEW IF EXISTS shelfon_vw_simple_top10_NoBrand;

CREATE VIEW IF NOT EXISTS shelfon_vw_simple_top10_NoBrand
AS
	...
	JOIN (select * from shelfon_collected_data_processed where active_flag = 'Y') AS SCD ON IC.ngid = SCD.ngid AND SCD.total_position <=10
