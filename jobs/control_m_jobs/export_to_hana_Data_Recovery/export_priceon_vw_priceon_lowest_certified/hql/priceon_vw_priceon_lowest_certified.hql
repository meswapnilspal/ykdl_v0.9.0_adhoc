...
DROP TABLE IF EXISTS priceon_vw_priceon_lowest_certified;

CREATE table IF NOT EXISTS priceon_vw_priceon_lowest_certified
AS
	SELECT 
		CAT.depth_fullname
, SKU.customer_code
...
, CC2.name AS name_collected_card_price
	...
	WHERE (PU.certifiedLowest in ('true','1') OR PU.uncertifiedLowest in ('true','1'));