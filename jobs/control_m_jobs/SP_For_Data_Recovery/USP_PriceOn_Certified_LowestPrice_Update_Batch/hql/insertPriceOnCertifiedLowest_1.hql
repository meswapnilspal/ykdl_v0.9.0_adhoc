...
MSCK REPAIR TABLE ${hivevar:hivedb}.priceon_piece_unit_processed5;
...
join (select * from ${hivevar:hivedb}.priceon_piece_unit_processed5 where active_flag = 'Y') PU on S.pid = PU.pid
...
join (select * from ${hivevar:hivedb}.priceon_piece_unit_processed5 WHERE active_flag = 'Y') as PU on S.pid = PU.pid
...
where CAST(CP.rtime AS date) = LP.rdate;

set hive.exec.dynamic.partition.mode=nonstrict;

MSCK REPAIR TABLE ${hivevar:hivedb}.priceon_piece_unit_processed5;

INSERT OVERWRITE TABLE ${hivevar:hivedb}.priceon_piece_unit_processed5 PARTITION(ingestion_date, active_flag)
...
FROM ${hivevar:hivedb}.priceon_piece_unit_seq_value1_PCL PU1
RIGHT JOIN ${hivevar:hivedb}.priceon_piece_unit_processed5 PU ON PU1.priceon_piece_unit_seq=PU.priceon_piece_unit_seq;
