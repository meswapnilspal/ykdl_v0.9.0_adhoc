...
#dateTo=`date '+%Y-%m-01'`
#dateFrom=$(date --date "$dateTo -1 month" +%Y-%m-01)
#dateFrom=`date '+%Y-%m-01'`
#dateTo=$(date --date "$dateTo +1 month" +%Y-%m-01)
dateFrom='2018-02-01'
dateTo='2018-03-01'

echo "Inserting into tb_korean_comparison table"
beeline -u ${hiveurl} --hivevar hivedb=${hivedb} -f ${hqlPath}/korean_skucd_mapping_conditions_1.hql
