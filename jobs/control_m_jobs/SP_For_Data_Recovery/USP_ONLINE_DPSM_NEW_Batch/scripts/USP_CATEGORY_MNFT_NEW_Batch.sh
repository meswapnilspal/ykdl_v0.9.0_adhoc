#!/bin/bash
#created by Aadil
#sh -x USP_CATEGORY_MNFT_NEW_Batch.sh
...
beeline -u ${hiveurl} --hivevar hivedb=${hivedb} -f ${hqlPath}/createTableMNFT_NEW.hql
count1=`beeline -u ${hiveurl} hive -e  "USE ${hivedb}; select count(*) from ${hivedb}.TB_CATEGORY_MNFT_REPORT_NEW;"`
...
then
  ...
  targetDate=`beeline -u ${hiveurl} hive -e "USE ${hivedb};select CAST(DATE_ADD(MAX(target_date), 1) AS date) FROM ${hivedb}.TB_CATEGORY_MNFT_REPORT_NEW;"`
  targetDate=`echo $targetDate | cut -d "|" -f4`
fi
...
do
  ...
  echo "Executing insertTableMNFT_NEW.hql"
  beeline -u ${hiveurl} --hivevar hivedb=${hivedb} --hivevar loopTargetDate=$loopTargetDate2 -f ${hqlPath}/insertTableMNFT_NEW.hql
