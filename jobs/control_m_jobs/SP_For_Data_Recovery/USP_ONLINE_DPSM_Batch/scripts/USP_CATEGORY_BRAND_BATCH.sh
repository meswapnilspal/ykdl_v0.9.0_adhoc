...
#count1=`beeline -u ${hiveurl} hive -e "USE ${hivedb};select count(*) from ${hivedb}.TB_CATEGORY_BRAND_REPORT;"`
...
#else
#  targetDate=`beeline -u ${hiveurl} hive -e "USE ${hivedb};select CAST(DATE_ADD(MAX(target_date), 1) as date) FROM ${hivedb}.TB_CATEGORY_BRAND_REPORT;"`
#  targetDate=`echo $targetDate | cut -d "|" -f4`
#fi

loopTargetDate='20180101'
